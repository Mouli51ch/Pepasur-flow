...
class GameManager {
  constructor(blockchainService = null, socketManager = null) {
    ...
    this.blockchainService = blockchainService; // Blockchain service (EvmServiceSimple)
    this.socketManager = socketManager; // Reference to SocketManager
    this.stakingService = new StakingService(blockchainService); // Pass blockchain service
  ...
  async createGame(creatorAddress, stakeAmount, minPlayers, contractGameId = null, isPublic = false) {
    ...
    // Save to MongoDB for persistence and public lobby queries (fire-and-forget)
    // Don't await - save in background to avoid blocking response
    ...
    dbGame.save()
      .then(() => console.log(`💾 Game saved to database: ${gameId}`))
      .catch(error => console.warn('⚠️ Could not save game to database:', error.message));
  ...
  async recordPlayerStake(gameId, playerAddress, transactionHash) {
    ...
    if (contractGameId && !this.stakingService.stakedGames.has(contractGameId)) {
      ...
      this.stakingService.stakedGames.set(contractGameId, {
        ...
        players: [],
        totalStaked: '0', // Keep as string to avoid concatenation issues
        status: 'waiting',
    ...
    if (contractGameId && this.stakingService.stakedGames.has(contractGameId)) {
      ...
      if (!stakingGame.players.includes(playerAddress)) {
        ...
        // Convert to BigInt to avoid string concatenation
        const currentTotal = BigInt(stakingGame.totalStaked || 0);
        const stakeAmount = BigInt(game.stakeAmount);
        stakingGame.totalStaked = (currentTotal + stakeAmount).toString();
  ...
  async startGame(gameId) {
    ...
    // Start game on-chain (move from READY to IN_PROGRESS)
    if (game.onChainGameId && game.stakingRequired) {
      try {
        console.log(`🎮 Starting game on-chain for contract gameId: ${game.onChainGameId}`);
        const blockchainService = this.stakingService.blockchainService;
        if (blockchainService && blockchainService.startGameOnChain) {
          await blockchainService.startGameOnChain(game.onChainGameId);
        } else {
          console.log(`⚠️ Blockchain service not available or startGameOnChain not implemented`);
        }
      } catch (error) {
